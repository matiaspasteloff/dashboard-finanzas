...
export default function TransactionsPage() {
    ...
    const load = useCallback(async () => {
        setLoading(true);
        try {
            const [txs, cats] = await Promise.all([transactionService.getAll(), categoryService.getAll()]);
            setTransactions(txs);
            setCategories(cats);
        } finally {
            setLoading(false);
    ...
    return (
        <div className="space-y-4 animate-slide-up">
                    ...
                    <table className="w-full">
                        <thead>
                            <tr className="border-b border-slate-100">
                                <th
                                    className="text-left text-xs font-semibold text-slate-400 uppercase tracking-wider px-5 py-4 cursor-pointer hover:text-slate-600 transition-colors whitespace-nowrap"
                                    onClick={() => handleSort('date')}
                                >
                                    Fecha <SortIcon col="date" />
                                </th>
                                <th
                                    className="text-left text-xs font-semibold text-slate-400 uppercase tracking-wider px-5 py-4 cursor-pointer hover:text-slate-600 transition-colors"
                                ...
                                <th
                                    className="text-right text-xs font-semibold text-slate-400 uppercase tracking-wider px-5 py-4 cursor-pointer hover:text-slate-600 transition-colors"
                                        ...
                                        <tr>
                                            <td colSpan={5} className="py-16 text-center text-slate-400 text-sm">
                                                <div className="flex flex-col items-center gap-2">
                                                    ...
                                                    <p>No hay transacciones{search ? ` para "${search}"` : ''}</p>
                                                    {!search && (
                                                        <button onClick={() => setModalOpen(true)} className="text-brand-500 font-semibold mt-1 hover:underline">
                                                            Crear la primera →
                                                        </button>
                                                    )}
                                                </div>
                    ...
                    <div className="px-5 py-3 border-t border-slate-50 flex items-center justify-between text-xs text-slate-400">
                        ...
                        <span className="num">
                            ...
                            <span className="font-semibold text-slate-600">
                                {formatARS(
                                    filtered.reduce((s, t) => {
                                        const isIncome = catMap[t.categoryId]?.type === 'INCOME';
                                        return s + (isIncome ? t.amount : -t.amount);
                                    }, 0)
                                )}
                            </span>
                        </span>
            ...
            <TransactionModal
                ...
                onSuccess={load}
                onCreate={transactionService.create}
            />
        </div>
    );
}
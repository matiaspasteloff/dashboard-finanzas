...
export default function Sidebar({ mobileOpen, onClose }: SidebarProps) {
    return (
        <>
            ...
            <aside
                className={`
          fixed top-0 left-0 z-30 h-full w-60 bg-sidebar flex flex-col
                    ...
                    {navItems.map(({ to, label, icon }) => (
                        ...
                        >
                            {({ isActive }) => (
                                <>
                                    <span
                                        className={`transition-colors duration-200 ${isActive ? 'text-brand-200' : 'text-slate-500 group-hover:text-slate-300'
                                            }`}
                                    >
                                        {icon}
                                    </span>
                ...
                <div className="px-3 py-4 border-t border-white/5">
                    <div className="bg-white/5 rounded-xl p-4">
                        ...
                        <p className="text-slate-500 text-xs leading-relaxed">
                            Registrar gastos diariamente mejora tu control financiero en un 40%.
                        </p>
                    </div>
                </div>
            </aside>

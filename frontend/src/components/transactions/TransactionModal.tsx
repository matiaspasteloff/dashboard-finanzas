...
export default function TransactionModal({ open, onClose, onSuccess, onCreate }: TransactionModalProps) {
    ...
    useEffect(() => {
        if (open) {
            ...
            setForm({ amount: '', description: '', categoryId: '', type: 'EXPENSE', isRecurring: false, recurrenceFrequency: 'MONTHLY' });
            setError('');
        }
    }, [open]);
                ...
                <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
                            ...
                            {(['INCOME', 'EXPENSE'] as const).map(type => {
                                ...
                                return (
                                    <button key={type} type="button" onClick={() => setForm(f => ({ ...f, type }))}
                                        className={`flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-semibold border-2 transition-all duration-200 ${isActive ? isIncome ? 'bg-income-bg border-income-DEFAULT text-income-text' : 'bg-expense-bg border-expense-DEFAULT text-expense-text' : 'bg-white border-slate-200 text-slate-500 hover:border-slate-300'}`}>
                    ...
                    <div>
                        ...
                        <div className="relative">
                            ...
                            <input type="number" min="0.01" step="0.01" placeholder="0.00" required value={form.amount}
                    ...
                    <div>
                        ...
                        <input type="text" placeholder="Ej: Supermercado, Sueldo, Netflix..." required value={form.description}
                    ...
                    <div className="bg-slate-50 rounded-xl px-4 py-3 border border-slate-100">
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-sm font-medium text-slate-700">Transacción recurrente</p>
                                <p className="text-xs text-slate-400 mt-0.5">Se repite periódicamente</p>
                            </div>
                            <button type="button" onClick={() => setForm(f => ({ ...f, isRecurring: !f.isRecurring }))}
                                className={`relative w-10 h-5 rounded-full transition-colors duration-200 focus:outline-none ${form.isRecurring ? 'bg-brand-500' : 'bg-slate-300'}`}>
                                <span className={`absolute top-0.5 left-0.5 w-4 h-4 rounded-full bg-white shadow transition-transform duration-200 ${form.isRecurring ? 'translate-x-5' : 'translate-x-0'}`} />
                            </button>
                        </div>
                        ...
                        {form.isRecurring && (
                            <div className="mt-3 pt-3 border-t border-slate-200">
                                ...
                                <div className="grid grid-cols-3 gap-2">
                                    {FREQUENCIES.map(f => (
                                        <button key={f.value} type="button" onClick={() => setForm(frm => ({ ...frm, recurrenceFrequency: f.value }))}
                                            className={`py-2 rounded-lg text-xs font-semibold border-2 transition-all ${form.recurrenceFrequency === f.value ? 'bg-brand-50 border-brand-500 text-brand-600' : 'bg-white border-slate-200 text-slate-500 hover:border-slate-300'}`}>
                                            {f.label}
                                        </button>
                                    ))}
                                </div>
                    ...
                    {error && (
                        <div className="bg-expense-bg border border-expense-DEFAULT/20 text-expense-text rounded-xl px-4 py-3 text-sm animate-fade-in flex items-center gap-2">
                    ...
                    <div className="flex gap-3 pt-1">
                        ...
                        <button type="submit" disabled={loading || filtered.length === 0}
